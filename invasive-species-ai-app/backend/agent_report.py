...
def build_agent():
    ...
    # Como usamos o Ollama (LLM Local), não é estritamente necessária uma chave OpenAI.
    # Alteramos o modelo para "llama3.2" (ou "llama3.1") que suporta "tool calling" nativo.
    model = ChatOllama(
        model="llama3.2",  # Llama 3.2 suporta nativamente a chamada de ferramentas (tools)
        temperature=0,
    )
